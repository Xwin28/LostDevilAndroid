...
public class ThirdPersonOrbitCamBasic : MonoBehaviour 
{
	...
	void Update()
	{
		...
		if (Input.touchCount > 0 || Input.GetMouseButton(0))
		{
				...
				// Mouse:
				angleH += Mathf.Clamp(SimpleInput.GetAxis("H"), -1, 1) * horizontalAimingSpeed;
				angleV += Mathf.Clamp(SimpleInput.GetAxis("V"), -1, 1) * verticalAimingSpeed;
				// Joystick:
				angleH += Mathf.Clamp(SimpleInput.GetAxis("H"), -1, 1) * 60 * horizontalAimingSpeed * Time.deltaTime;
				angleV += Mathf.Clamp(SimpleInput.GetAxis("V"), -1, 1) * 60 * verticalAimingSpeed * Time.deltaTime;
