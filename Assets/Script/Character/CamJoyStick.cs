...
{
	public class CamJoyStick : MonoBehaviour, ISimpleInputDraggable
	{
		public enum MovementAxes { XandY, X, Y };

		public SimpleInput.AxisInput xAxis = new SimpleInput.AxisInput("H");
		public SimpleInput.AxisInput yAxis = new SimpleInput.AxisInput("V");
...
#pragma warning restore 0649
		...
		private void OnUpdate()
		{
			...
			if (joystickHeld)
				opacity = 0;// Mathf.Min(1f, opacity + Time.unscaledDeltaTime * 4f);
			else
				opacity = 0;// Mathf.Max(0f, opacity - Time.unscaledDeltaTime * 4f);
